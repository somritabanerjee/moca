...
class ALPaCA:
    ...
    def construct_model(self):
        ...
        if self.sigma_eps is list:
            self.SigEps = torch.diag(torch.tensor(np.array(self.sigma_eps)))
        else:
            self.SigEps = self.sigma_eps*torch.eye(self.y_dim)
        self.SigEps = torch.reshape(self.SigEps, (1,1,self.y_dim,self.y_dim))
